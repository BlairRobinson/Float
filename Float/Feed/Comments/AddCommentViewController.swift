...
class AddCommentViewController: UIViewController, UITextViewDelegate {
    ...
    var floatId: String = CommentTableViewController.floatId
    ...
    func textViewDidBeginEditing(_ textView: UITextView) {
        if floatTextView.text == "Write your comment here..." {
            floatTextView.text = ""
    ...
    @IBAction func addCommentBtnPressed(_ sender: Any) {
        ...
        Database.database().reference().child("floats").child(floatId).child("comments").observeSingleEvent(of: .value) { (snapshot) in
    ...
    func loadNextVC() {
        let vc = self.storyboard?.instantiateViewController(withIdentifier: "Comments") as? UINavigationController
